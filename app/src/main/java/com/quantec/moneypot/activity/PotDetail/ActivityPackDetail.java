...
import com.quantec.moneypot.activity.Main.Fragment.Tab1.Adapter.AdapterStockList;
...
import com.quantec.moneypot.databinding.ActivityPackDetailBinding;
...
public class ActivityPackDetail extends AppCompatActivity {
    ...
    String code, title;
    int position, follow, newsPlus, newsMinus, nowPage, maxPage, type;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_pack_detail);

        loadingCustomMakingPort = new DialogLoadingMakingPort(ActivityPackDetail.this, "주식투자는 단기적 수익을 쫒기 보다는\n장기적으로 보아야 성공할 수 있습니다.");
        ...
        preChartBt = binding.includePackDetail.aBt;
        changedChartBt(preChartBt);

        modelStockLists = new ArrayList<>();
        adapterStockList = new AdapterStockList(modelStockLists, this);
        binding.includePackDetail.stockRecyclerView.setAdapter(adapterStockList);

        Call<ModelCustomDetail> getReList = RetrofitClient.getInstance().getService().getDetailData("application/json", 1, code, "all", 701);
        getReList.enqueue(new Callback<ModelCustomDetail>() {
            ...
            public void onResponse(Call<ModelCustomDetail> call, Response<ModelCustomDetail> response) {
                if (response.code() == 200) {
                    ...
                    binding.includePackDetail.progress.setProgressBackgroundColor(getResources().getColor(R.color.life_line));
                    binding.includePackDetail.progress.setProgressColor(getResources().getColor(R.color.c_4e7cff));
                    binding.includePackDetail.progress.setProgress(newsPlus);
                    binding.includePackDetail.progressText.setText(String.valueOf(newsPlus));

                    binding.includePackDetail.progress2.setProgressBackgroundColor(getResources().getColor(R.color.life_line));
                    binding.includePackDetail.progress2.setProgressColor(getResources().getColor(R.color.c_a8a7a7));
                    binding.includePackDetail.progress2.setProgress(newsMinus);
                    binding.includePackDetail.progressText2.setText(String.valueOf(newsMinus));

                    if((newsPlus==0) && (newsMinus==0)){
                        binding.includePackDetail.aiLayout2.setVisibility(View.GONE);
                        binding.includePackDetail.aiEmpty.setVisibility(View.VISIBLE);
                    }else{

                        binding.includePackDetail.aiLayout2.setVisibility(View.VISIBLE);
                        binding.includePackDetail.aiEmpty.setVisibility(View.GONE);

                        if(newsPlus > newsMinus){
                            binding.includePackDetail.aiImage.setBackgroundResource(R.drawable.img_positive);
                            binding.includePackDetail.aiText.setText("긍정적");
                            binding.includePackDetail.aiText.setTextColor(getResources().getColor(R.color.c_4e7cff));
                        }else if(newsPlus < newsMinus){
                            binding.includePackDetail.aiImage.setBackgroundResource(R.drawable.img_negative);
                            binding.includePackDetail.aiText.setText("부정적");
                            binding.includePackDetail.aiText.setTextColor(getResources().getColor(R.color.c_9a9a9a));
                        }else{
                            binding.includePackDetail.aiImage.setBackgroundResource(R.drawable.img_neutral);
                            binding.includePackDetail.aiText.setTextColor(getResources().getColor(R.color.c_cccccc));
                            binding.includePackDetail.aiText.setText("평가동일");
                        }
                    }


                    binding.includePackDetail.price.setText(MoneyFormatToWon.moneyFormatToWon(response.body().getContent().getCoreData().getCore().getTotPrice()));
                    binding.includePackDetail.rate.setText(String.format("%.2f", response.body().getContent().getCoreData().getCore().getRate())+"%");
                    binding.includePackDetail.investPrice.setText(MoneyFormatToWon.moneyFormatToWon(response.body().getContent().getCoreData().getCore().getMinPrice()));
                    if(response.body().getContent().getCoreData().getCore().getRate() < 0){
                        binding.includePackDetail.rate.setTextColor(getResources().getColor(R.color.c_4e7cff));
                    }else{
                        binding.includePackDetail.rate.setTextColor(getResources().getColor(R.color.c_f02654));
                    }


                    modelStockLists.add(new ModelStockList(true, "", 0));

                    for(int index = 0; index < response.body().getContent().getTags().size(); index++){

                        modelStockLists.add(new ModelStockList(false,
                                response.body().getContent().getTags().get(index).getName(),
                                response.body().getContent().getTags().get(index).getWeight()*100));
                    }
        ...
        binding.includePackDetail.articleRecyclerView.setHasFixedSize(true);
        layoutManager = new LinearLayoutManager(this);
        binding.includePackDetail.articleRecyclerView.setLayoutManager(layoutManager);

        modelArticles = new ArrayList<>();
        modelArticlesAll = new ArrayList<>();

        adapterArticle = new AdapterArticle(modelArticles, this);
        binding.includePackDetail.articleRecyclerView.setAdapter(adapterArticle);

        binding.includePackDetail.articleRecyclerView.setLayoutManager(new LinearLayoutManager(this){
            @Override
        ...
        binding.includePackDetail.stockRecyclerView.setHasFixedSize(true);
        layoutManager1 = new LinearLayoutManager(this);
        binding.includePackDetail.stockRecyclerView.setLayoutManager(layoutManager1);


        binding.includePackDetail.likeEnterRecyclerView.setHasFixedSize(true);
        layoutManager2 = new LinearLayoutManager(this, RecyclerView.HORIZONTAL, false);
        binding.includePackDetail.likeEnterRecyclerView.setLayoutManager(layoutManager2);

        modelLikeEnters = new ArrayList<>();
        adapterLikeEnter = new AdapterLikeEnter(modelLikeEnters, this);

        binding.includePackDetail.likeEnterRecyclerView.addItemDecoration(new DecorationItemHorizontal(this, 10));

        binding.includePackDetail.likeEnterRecyclerView.setAdapter(adapterLikeEnter);
        ...
        RxView.clicks(binding.includePackDetail.m1Bt).throttleFirst(1500, TimeUnit.MILLISECONDS).subscribe(empty -> {
            if(monTh1) {
                loading.setVisibility(View.VISIBLE);
                changedChartBt(binding.includePackDetail.m1Bt);
                monTh1 = false;
                chartData("one");
            }else{
                changedChartBt(binding.includePackDetail.m1Bt);
        ...
        RxView.clicks(binding.includePackDetail.m3Bt).throttleFirst(1500, TimeUnit.MILLISECONDS).subscribe(empty -> {
            if(monTh3){
                loading.setVisibility(View.VISIBLE);
                changedChartBt(binding.includePackDetail.m3Bt);
                monTh3 = false;
                chartData("thr");
            }else{
                changedChartBt(binding.includePackDetail.m3Bt);
        ...
        RxView.clicks(binding.includePackDetail.m6Bt).throttleFirst(1500, TimeUnit.MILLISECONDS).subscribe(empty -> {
            if(monTh6){
                loading.setVisibility(View.VISIBLE);
                changedChartBt(binding.includePackDetail.m6Bt);
                monTh6 = false;
                chartData("six");
            }else{
                changedChartBt(binding.includePackDetail.m6Bt);
        ...
        RxView.clicks(binding.includePackDetail.aBt).throttleFirst(1500, TimeUnit.MILLISECONDS).subscribe(empty -> {
            changedChartBt(binding.includePackDetail.aBt);
            drawLineChart(entries);
        });
        ...
        adapterArticle.setArticleAddBtClick(new AdapterArticle.ArticleAddBtClick() {
            ...
            public void onClick(int position) {
                ...
                if(nowPage >= 10){
                    ...
                    Intent intent = new Intent(ActivityPackDetail.this, ActivityArticle.class);
        ...
        adapterArticle.setArticleEnterClick(new AdapterArticle.ArticleEnterClick() {
            ...
            public void onClick(int position) {
                ...
                Intent intent = new Intent(ActivityPackDetail.this, ActivityWebViewArticle.class);
        ...
        adapterLikeEnter.setLikeEnterClick(new AdapterLikeEnter.LikeEnterClick() {
            ...
            public void onClick(int position) {
                ...
                Intent intent = new Intent(ActivityPackDetail.this, ActivitySingleDetail.class);
    ...
    private void drawLineChart(List<Entry> chartData){
        ...
        CustomMarkerView marker = new CustomMarkerView(ActivityPackDetail.this, R.layout.item_chart_marker);
            ...
            public void onResponse(Call<Object> call, Response<Object> response) {
                if (response.code() == 200) {
                    ...
                    if(following == 1){
                        follow = 1;
                        binding.followBt.setImageDrawable(getResources().getDrawable(R.drawable.ic_star_on_blue));
                        Toast.makeText(ActivityPackDetail.this.getApplicationContext(), "팔로우", Toast.LENGTH_SHORT).show();
                    }else{
                        follow = 0;
                        binding.followBt.setImageDrawable(getResources().getDrawable(R.drawable.ic_star_off_gray));
                        Toast.makeText(ActivityPackDetail.this.getApplicationContext(), "팔로우 취소", Toast.LENGTH_SHORT).show();
                    }
                }
            }
            @Override
